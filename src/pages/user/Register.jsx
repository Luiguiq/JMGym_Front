  ...
  return (
    <main className="min-h-screen bg-slate-50 md:grid md:place-items-center md:bg-gradient-to-b md:from-brand-50 md:to-brand-100">
      <section className="min-h-screen bg-[radial-gradient(circle_at_50%_0%,rgba(255,255,255,0.95),transparent_22rem),linear-gradient(180deg,#fbfdff_0%,#edf8ff_100%)] px-6 py-8 md:grid md:w-full md:grid-cols-[minmax(360px,0.8fr)_minmax(500px,0.85fr)] md:items-center md:gap-x-28 md:bg-[radial-gradient(circle_at_16%_20%,rgba(255,255,255,0.92),transparent_26rem),linear-gradient(180deg,#f7fcff_0%,#e8f7ff_100%)] md:px-[8vw] md:py-16">
        ...
        <form className="mx-auto mt-6 grid w-full max-w-xl gap-4 md:m-0 md:max-w-[620px] md:gap-5">
          <InputField icon="👤" label="Nombre completo" placeholder="Maria Garcia Lopez" />
          <InputField icon="🪪" label="DNI" placeholder="12345678" />
          <InputField icon="✉️" label="Correo electronico" placeholder="maria@correo.com" type="email" />
          <InputField icon="🔒" label="Contraseña" placeholder="Minimo 8 caracteres" type="password" />

          <label className="mt-1 flex items-start gap-3 leading-relaxed text-slate-500">
            <input className="mt-1 h-5 w-5 shrink-0 accent-brand-600" type="checkbox" defaultChecked />
          ...
          <div className="mt-3 grid gap-5 text-center">
            <button className="min-h-14 rounded-2xl bg-gradient-to-r from-brand-500 to-brand-600 font-extrabold text-white shadow-soft md:min-h-16 md:text-lg" type="submit">Registrarme</button>
...
function InputField({ icon, label, placeholder, type = 'text' }) {
  return (
    <label className="grid gap-2 font-bold text-slate-500">
      ...
      <div className="flex min-h-14 items-center gap-3 rounded-2xl border-2 border-brand-100 bg-white px-4 shadow-[0_10px_24px_rgba(9,105,163,0.06)] md:min-h-16">
        ...
        <input className="w-full bg-transparent outline-none" type={type} placeholder={placeholder} />
      </div>
    </label>
  );
}
